...
export async function POST(request: NextRequest) {
  try {
    const { subject, body, taskStatus, persona } = await request.json();
    ...
    if (!geminiKey || geminiKey === 'mock-key') {
      return NextResponse.json({ draft: getFallbackDraft(subject, taskStatus, persona) });
    }
...
function getFallbackDraft(subject: string, taskStatus: string, persona: string): string {
  const isCompleted = taskStatus === 'completed';
  
  if (persona === 'The Motivator') {
    return `Hi team!\n\nJust wanted to send a quick update on this. ${isCompleted ? "Woohoo! I've already tackled the task and got it fully completed! Let me know if there's anything else we need to jump on." : "I'm on it! I've logged this task in my dashboard queue and am actively working on it. Will send over the details the moment it is ready. Let's do this!"}\n\nBest,\nCharlie`;
  }
  
  if (persona === 'The Auditor') {
    return `To Whom It May Concern,\n\nI am writing to confirm receipt of your correspondence regarding "${subject}".\n\nThis is to notify you that the associated action items are currently: ${isCompleted ? "VERIFIED & COMPLETED. All records have been reconciled." : "PENDING REVIEW. Work is scheduled and progress is tracked in the system dashboard."}\n\nRespectfully,\nCharles`;
  }

  // Executive Minimalist default
  return `Hi,\n\nThanks for reaching out. ${isCompleted ? "The tasks associated with this request have been completed." : "I have added these tasks to my queue and will update you shortly."}\n\nBest,\nCharlie`;
}
